...
SlackTransport = Callable[[str, dict[str, str]], dict[str, Any]]
...
class SlackWebApiReplyHost:
    ...
    def replies(self, *, channel_id: str, parent_message_ts: str) -> list[dict]:
        ...
        response = self._call("conversations.replies", {"channel": channel_id, "ts": parent_message_ts})
        ...
        return [
            ...
            if isinstance(item, dict) and str(item.get("ts", "")) != parent_message_ts
        ]
    ...
    def _call(self, method: str, payload: dict[str, str]) -> dict[str, Any]:
        try:
            response = self._transport(method, payload)
